...
pub unsafe fn iterate_tensors(
    ...
    for tensor_idx in 0..tensor_count {
        ...
        let mut return_value = match tensor_info.dtype {
            nrt::nrt_dtype_NRT_DTYPE_FLOAT32 => Output::Float32(Rc::new(Vec::with_capacity(
                tensor_info.size / std::mem::size_of::<f32>(),
            ))),
            nrt::nrt_dtype_NRT_DTYPE_UINT8 => Output::Bool(Rc::new(Vec::with_capacity(tensor_info.size))),
...
pub unsafe extern "C" fn handler_save_outputs(
    ...
    *return_value = match *return_value {
        Output::Float32(ref mut _rc) => {
            ...
            Output::Float32(Rc::new(v))
        }
        Output::Bool(ref mut _rc) => {
...
/// 5. The `device_id` should always be 0 for processes running on 1 device. Use `device_id` to set
///    device if running a process using several devices.
///
/// If these invariants are not upheld, the function may cause undefined behavior or memory corruption.
